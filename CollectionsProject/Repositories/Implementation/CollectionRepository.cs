...
namespace CollectionsProject.Repositories.Implementation
{
    public class CollectionRepository : AbstractRepository<Collection>, ICollectionRepository
    {
        ...
        {
            return await db.Collections.Include(c => c.User).Include(c => c.AddFields)
                .FirstOrDefaultAsync(c => c.CollectionId.ToString() == id);
        }
        ...
        {
            return await db.Collections.Include(c => c.User).OrderByDescending(c => c.Items.Count)
        ...
        {
            return await db.Collections.Include(c => c.User).OrderBy(c => c.CollectionId)
        ...
        {
            return await db.Collections.Include(c => c.User).Where(c => c.UserId == id)
                .OrderBy(c => c.CollectionId).Skip(itemsToSkip).Take(itemsToTake).ToListAsync();
        }
